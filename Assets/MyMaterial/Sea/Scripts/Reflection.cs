...
namespace UnityEngine.Rendering.Universal {
	...
	public class Reflection : MonoBehaviour {
		...
		readonly private int _planarReflectionTextureId = Shader.PropertyToID( "_ReflectTex" );
		...
		private void runPlannarReflection( ScriptableRenderContext context, Camera camera ) {
			...
			if( _reflectionCamera == null ) {
				// create reflection camera, not active and hidden
				_reflectionCamera = CreateReflectCamera();
			}

			// set Lod, turn off fog, invert culling
			...
			// reset to previous camera settings
		...
		// ref: https://zhuanlan.zhihu.com/p/493766119
		...
		private void UpdateReflectionCamera( Camera curCamera ) {
			...
			// move the plane below the water surface?
			...
			// move the reflection camera to the other side of the plane
			var pos = curCamera.transform.position;
			var newPos = pos - 2 * Vector3.Dot( planeNormal, pos - planePos ) * planeNormal;
		...
		private Camera CreateReflectCamera() {
			...
			//相机渲染顺序[-100, 100]，主相机为0，负数为提前渲染，正数为延后渲染
